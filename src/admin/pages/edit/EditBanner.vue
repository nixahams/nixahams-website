<template>
  <div class="subroute_area">
    <div id="bannerPreview_container" :class="colorClass">
      ...
      <div id="bannerPreview_type">{{ bannerData.type }}:</div>
    ...
    <form @submit.prevent="submitBanner">
      <div id="bannerInput_container">
        <div class="bannerInputA">
          ...
          <label class="switch">
            <input name="active" type="checkbox" v-model="bannerShow" />
        ...
        <div class="bannerInputA">
          Icon Type
          <select
            @change="bannerData.icon = $event.target.value"
            name="icon"
            id=""
            class="bannerOption"
          >
        ...
        <div class="bannerInputA">
          Signal Type
          <select
            @change="bannerData.type = $event.target.value"
            name="signal"
            id=""
            class="bannerOption"
          >
        ...
        <div class="bannerInputA">
          ...
          <div name="color" id="radioField">
            <div
              @click="updateColor(1, $event)"
              class="radioColor"
              id="unchecked"
            ></div>
            <div
              @click="updateColor(2, $event)"
              class="radioColor"
              id="unchecked"
            ></div>
            <div
              @click="updateColor(3, $event)"
              class="radioColor"
              id="checked"
            ></div>
          </div>
        ...
        <div class="bannerInputB">
          ...
          <span> ( {{ charUsed }} / 160 ) characters used </span>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      editURL:
        "https://us-east-1.aws.data.mongodb-api.com/app/app-0-yyrfg/endpoint/edit/admin_message",
      bannerData: {
        type: "type",
        icon: "circle-info",
        text: "message",
  ...
  methods: {
    ...
    updateColor(selected, div) {
      ...
      for (let i = 0; i < 3; i++) {
        this.colorGroup[i].id = "unchecked";
      }
...
#checked {
  box-shadow: 0px 0px 0px 1px #1a1a1a, 0px 0px 0px 3px rgba(255, 255, 255, 1);
}
#unchecked {
  box-shadow: 0px 0px 0px 1px #1a1a1a, 0px 0px 0px 3px rgba(255, 255, 255, 0.4);
}
.radioColor:nth-child(1) {
  background: -webkit-linear-gradient(315deg, #db1f1f, #ff6161 60%);
}
.radioColor:nth-child(2) {
  background: -webkit-linear-gradient(315deg, #0368e2, #75b3ff 60%);
}
.radioColor:nth-child(3) {
  background: -webkit-linear-gradient(315deg, #df7920, #ffcd61 60%);
}
.radioColor {
  width: 30px;
  height: 30px;
...
.banner_red {
  background: -webkit-linear-gradient(45deg, #db1f1f 40%, #ff6161);
}
.banner_blue {
  background: -webkit-linear-gradient(45deg, #0368e2 40%, #75b3ff);
}
.banner_orange {
  background: -webkit-linear-gradient(45deg, #df7920 40%, #ffcd61);
}
#bannerPreview_exit {
  cursor: pointer;
}
