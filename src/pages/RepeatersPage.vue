<template>
  <div id="repeaters_page">
    <div
      ...
      @click="exitFullScreenImage"
      id="fullScreen_clickOut"
    ></div>
    ...
    <CardArrow
      ...
      v-if="asyncProp"
      :repeater_list="passList"
    />
    <div :class="desc_class" id="descriptive_parent">
      <div id="more_images">
        <ActiveImages
          @emitFullScreenImage="fullScreenImage(result)"
          v-for="(result, index) in image_array"
          :key="index"
          :index="index"
          :image="result"
        />
      ...
      <div id="repeater_filter">
        ...
        <input
          type="text"
          @click="user_active($event.target.values)"
          @input="user_typing($event.target.value)"
        ...
        <div v-if="result_visible" id="search_results">
          <ResultOption
            @option_selected="op_selected"
            v-for="(res, index) in result_list"
            :key="index"
            :res="res"
            :filter="filter_by"
          />
      ...
      <div id="desc_info_desc">
        <span
          :key="lineNumber"
          v-for="(line, lineNumber) of long_desc.split('<br>')"
          >{{ line }}</span
        >
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    user_typing(text) {
      ...
      this.result_visible = true;
      let str = text;
      let stringArray = str.split(/(\s+)/).filter(function (e) {
    ...
    getRepeaters(VueObj) {
      const URL =
        "https://us-east-1.aws.data.mongodb-api.com/app/app-0-yyrfg/endpoint/repeaters";
      axios
        .get(URL)
